...
survey_data <- 
  ...
  mutate(
    ...
    edu = case_when(
      ...
      edu %in% c(3, 4, 5, 6, 7, 8) ~ 2, #college/trades
      edu %in% c(9, 10, 11) ~ 3, #University degree
      edu == 12 ~ NA_real_
    )
  ) %>%
...
agegrp_summary <- survey_data %>%
  count(agegrp) %>%
  mutate(percent = n / sum(n) * 100)

print(agegrp_summary)
