...
For the Liberal Party model, we specified: $$logit(P(Y_i=1))=\beta_0+\beta_1(age_i)+\beta_2(prov_i)+\beta_3(gender_i)+\beta_4(edu_i),$$where $Y_i$ indicates whether respondent $i$ intends to vote Liberal (1 = yes, 0 = no), $\beta_0$ is the intercept, and $\beta_1$, $\beta_2$, $\beta_3$, and $\beta_4$ represent the effects of age, province, gender, and education respectively. Parallel logistic regression models were also fit for the Conservative Party and NDP.
...
Cells were defined by all combinations of age × province × gender × education, since all four predictors were included in the logistic models (they are expected to be associated with political preference) and were consistently measured across both the survey and census datasets.
