...
This report sought to estimate national popular vote shares for the upcoming Canadian federal election by combining logistic regression with post-stratification. The approach relied on demographic predictors available in both the survey and the census—age, province, gender, and education—to model voting intention for the Liberal Party, Conservative Party, and NDP. The resulting probabilities were then weighted by census cell counts to construct population-adjusted estimates of national support levels.

The post-stratified results suggest predicted popular vote shares of approximately r results_table$PredictedShare[results_table$Party == "liberal"]% for the Liberal Party, r results_table$PredictedShare[results_table$Party == "conservative"]% for the Conservative Party, and r results_table$PredictedShare[results_table$Party == "ndp"]% for the NDP. Overall, the distribution indicates a relatively competitive environment in which no party achieves a decisive advantage. The relative ordering of predicted support—Liberals leading, followed by Conservatives, then the NDP—is broadly consistent with patterns observed in recent federal elections, suggesting that the combined modeling and post-stratification approach produces plausible national estimates.
...
Despite these informative patterns, the analysis remains constrained by several limitations. First, the models omit relevant predictors such as income, urbanicity, occupation, and race. Although the CES contains additional demographic variables such as income and ethnicity, these variables were not included because the corresponding Census variables are coded differently or cannot be directly aligned. Post-stratification requires consistent definitions across both datasets, so only age, province, gender, and education were retained. Second, survey data inevitably contain sampling noise and potential nonresponse bias, which post-stratification can mitigate but not entirely eliminate. Third, the study reports point estimates without any accompanying measures of uncertainty. The absence of confidence intervals or standard errors limits the ability to assess the stability of the predictions or compare competing electoral scenarios.
...
Since the CES 2021 data is publicly available, describe whether or not the work completed in your report needs Research Ethics Board approval for the report the be made publicly available. Be sure to specifically discuss the privacy of human participants in this study.-->
