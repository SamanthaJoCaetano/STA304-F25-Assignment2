...
Canadian federal elections are heavily shaped by regional and demographic differences, making it difficult to estimate how national voting outcomes will unfold. In this report, we aim to predict the popular vote—the total percentage of votes received by each party nationwide—for the upcoming Canadian federal election (tentatively 2029). We merge survey answers from the Canadian Election Study (Stephenson et al., 2022) with population data from the 2021 Canadian Census to create a modified estimate of nationwide voting tendencies (Stephenson et al., 2022; Statistics Canada & Statistics Canada, 2023).

Being able to predict voting outcomes is very useful as it can highlight the broader political environment, helping researchers and policymakers better understand the views of the voters by providing numerical insights regarding party preferences. Even though Canada selects its government using a first-past-the-post method, trends in the popular vote are extensively covered in media and academic circles, as they indicate national backing for significant political parties and uncover prolonged ideological changes among Canadians.

To answer our research question — What will the popular vote distribution look like for the Liberal, Conservative, and NDP parties in the 2029 federal election? — we apply post-stratification, a statistical method that corrects for survey non-representativeness by weighting model predictions according to the true demographic distribution of the population. Key demographic variables such as age and province are expected to play an influential role in voting preferences based on existing political trends.

Prior to conducting our analysis, we hypothesized that: (1) older age groups will be more likely to vote for the Conservative Party; (2) younger voters will show greater support for the NDP; and (3) voting preferences will vary substantially across provinces. The remainder of this report describes the data sources, statistical methods, results of the post-stratified vote share estimates, and an interpretation of findings.
...
<<<<<<< HEAD
The analysis uses two publicly available datasets: the 2021 Canadian Election Study (CES) and the 2021 Canadian Census. The CES is a nationally distributed survey that collects information on Canadians’ political attitudes, vote intention, and demographic characteristics during federal election cycles. It utilizes sampling and weighting techniques driven by probability to ensure extensive participation from eligible voters across different areas (Stephenson et al., 2022). The 2021 Census offers comprehensive demographic figures for the Canadian populace and is conducted by Statistics Canada every five years to assess age, gender, education, and geographical distribution (Statistics Canada & Statistics Canada, 2023). Together, the CES and Census allow us to combine behavioural data (voting intention) with population counts to correct for survey non-representativeness using post-stratification.
=======
analysis uses two publicly available datasets: the 2021 Canadian Election Study (CES) and the 2021 Canadian Census. The CES is a nationally distributed survey that collects information on Canadians’ political attitudes, vote intention, and demographic characteristics during federal election cycles. It utilizes sampling and weighting techniques driven by probability to ensure extensive participation from eligible voters across different areas (CES, 2021). The 2021 Census offers comprehensive demographic figures for the Canadian populace and is conducted by Statistics Canada every five years to assess age, gender, education, and geographical distribution (Statistics Canada, 2021). Together, the CES and Census allow us to combine behavioural data (voting intention) with population counts to correct for survey non-representativeness using post-stratification
>>>>>>> 89d1cf93f17067dbb2db3924deee85a378ffa14d
...
Figure 1 displays the distribution of CES respondents across age categories. The figure shows that majority of the CES respondents were in their late-30s to late-60s compared to young adults and seniors, highlighting the vitality of adjusting estimates based on population data. Figure 2 illustrates the proportion of each age category indicating a desire to vote Liberal. Support peaks among younger and middle-aged respondents before declining in the oldest categories. Figures 3 and 4 show how preference for the Conservative and NDP parties differ amongst age categories; Conservatives are most supported by older age groups, whereas the NDP party is most supported by young respondents. These illustrative visualizations encourage the addition of age and province in the regression model and emphasize the demographic gradients that post-stratification seeks to address.
...
Cells were defined by all combinations of age group × province, because both variables are strongly associated with political preference and are available in both the survey and census datasets. Other variables (e.g., education level) were not included because they were either unavailable or inconsistently measured across data sources.
...
Even with these advantages, the analysis has multiple shortcomings. Initially, the models rely solely on age and province as predictors, which implies that significant variables like education, income, urbanicity, and race/ethnicity are overlooked due to either data availability issues or inconsistencies between the survey and the census. Additionally, survey data naturally carry uncertainty, and sampling bias can remain even following post-stratification. Ultimately, we did not calculate standard errors or confidence intervals for the final popular vote estimates, meaning the uncertainty linked to the predictions cannot be formally assessed.
