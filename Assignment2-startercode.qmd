...
To prepare both datasets for analysis, we recoded the CES variables so that they matched the structure and categories used in the Census data. From the CES, we extracted age, province, gender, education, and vote intention, and transformed these into categories consistent with Statistics Canada definitions. Age was grouped into 15 categories (from 18–19 through 85+), province codes were converted to standard provincial identifiers with the three territories combined into a single “North” category, gender was recoded as “Woman” or “Man,” and education was grouped into three levels: high school or less, some college, and a college degree. We also created binary indicators for Liberal, Conservative, and NDP vote intention. The Census dataset was then recoded using the same age groups, province categories, gender labels, and education levels so that each demographic cell aligned with the CES structure. Observations with missing or incompatible values were removed in both datasets. For the modeling stage, we focused on age and province because these variables are consistently measured in both sources, capture meaningful demographic variation in voting behavior, and support a straightforward post-stratification design.
...
The age distribution (Figure 1) shows that respondents are most commonly found in the middle age groups, with smaller counts in the youngest (18–19) and oldest (85+) categories. This uneven distribution highlights differences between the survey sample and the broader Canadian population and helps motivate the need for post-stratification later in the analysis.

Figures 2–4 show the proportion of respondents within each age group who reported intending to vote for the Liberal Party, Conservative Party, or NDP. These plots illustrate how support varies across age categories, providing context for why age is included as a predictor in the regression model. Across the full CES sample, approximately 27.2% support the Liberals, 24.8% support the Conservatives, and 18.6% support the NDP. These summary measures help establish the overall partisan composition of the sample before adjusting the estimates using population data.
